...
class Mclass_dr{
	...
	function show_all_medicine($sql){
			$conn = $this->connDB();
			$result = mysqli_query($conn ,$sql);
			$medicine = mysqli_fetch_all($result, MYSQLI_ASSOC);
			mysqli_free_result($result);
			mysqli_close($conn);
			return $medicine;
	}
	function show_PK($sql){
			$conn = $this->connDB();
			$result = mysqli_query($conn ,$sql);
			$result_card = mysqli_fetch_all($result, MYSQLI_ASSOC);
			mysqli_free_result($result);
			mysqli_close($conn);
			return $result_card;
							}
	...
	function exist_medicine($tenthuoc){
			$conn = $this->connDB();
			$sql = "select * from khothuoc where tenthuoc = '$tenthuoc'";
			$result = mysqli_query($conn ,$sql);
			$exist_medicine = mysqli_fetch_assoc($result);
			mysqli_free_result($result);
			mysqli_close($conn);
			return $exist_medicine;
		}
