...
function loginFromSocialCallBack($socialUser) {
    ...
    $result = mysqli_query($con, "Select `username`,`email`,`fullname` from `user` WHERE `email` ='" . $socialUser['email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `user` (`fullname`,`email`, `created_at`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', " . time() . ");");
        if (!$result) {
            echo mysqli_error($con);
            exit;
        }
        $result = mysqli_query($con, "Select `id_user`,`username`,`email`,`fullname` from `user` WHERE `email` ='" . $socialUser['email'] . "'");
    }
    if ($result->num_rows > 0) {
