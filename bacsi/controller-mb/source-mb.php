<?php
	class Mclass_mb{
		...
		function connDB(){
			...
			//$conn = mysqli_connect("sql210.byethost7.com", "b7_27616432", "loc!@#456", "tmdt");
			$conn -> set_charset("utf8");
			if(!$conn):
		...
		function show_infoPK_byid($id_phieukham){
			$conn = $this->connDB();
			$sql = "select * from phieukham where id_phieukham = '$id_phieukham'";
			$result = mysqli_query($conn ,$sql);
			$phieukham = mysqli_fetch_assoc($result);
			mysqli_free_result($result);
			mysqli_close($conn);
			return $phieukham; 
		}
		...
		function show_info_medicine($id_phieukham){
			$conn = $this->connDB();
